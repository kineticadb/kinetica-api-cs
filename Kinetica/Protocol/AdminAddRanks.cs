...
/// <para>* <see cref="AdminAddRanksRequest.hosts" /> would be an array
/// including 172.123.45.67 in the first two indices (signifying two ranks
/// being added to host 172.123.45.67) and 172.123.45.68 in the last index
/// (signifying one rank being added to host 172.123.45.67)</para>
/// <para>* <see cref="AdminAddRanksRequest.config_params" /> would be an array
/// of maps, with each map corresponding to the ranks being added in <see
/// cref="AdminAddRanksRequest.hosts" />. The key of each map would be the
...
public class AdminAddRanksRequest : KineticaData
{
    ...
    /// cref="AdminAddRanksRequest.options" />.</summary>
    ...
    public struct Options
    {
        ...
        /// cref="AdminAddRanksRequest.Options.FALSE">FALSE</see>.</para>
        /// </remarks>
    ...
    /// <summary>Array of host IP addresses (matching a hostN.address from the
    /// gpudb.conf file), or host identifiers (e.g. 'host0' from the gpudb.conf
    ...
    /// corresponds to the entry at the same index in the <see
    /// cref="AdminAddRanksRequest.config_params" />.</para></remarks>
    public IList<string> hosts { get; set; } = new List<string>();

    /// <summary>Array of maps containing configuration parameters to apply to
    /// the new ranks found in <see cref="AdminAddRanksRequest.hosts" />.
    /// </summary>
    ...
    /// target="_top">RAM Tiered Storage</a> sections in the gpudb.conf file,
    /// with the key exception of the 'rankN.host' settings in the Network
    /// section that will be determined by <see
    ...
    /// in <see cref="AdminAddRanksRequest.config_params" /> as the new rank
    /// number(s) are not allocated until the ranks have been added to the
    /// cluster. Each entry in this array corresponds to the entry at the same
    /// index in the <see cref="AdminAddRanksRequest.hosts" />. This array must
    /// either be completely empty or have the same number of elements as the
    /// <see cref="AdminAddRanksRequest.hosts" />.  An empty <see
    /// cref="AdminAddRanksRequest.config_params" /> array will result in the
    /// new ranks being set with default parameters.</para></remarks>
    ...
    /// <summary>Constructs an AdminAddRanksRequest object with the specified
    /// parameters.</summary>
    ///
    ...
    /// target="_top">RAM Tiered Storage</a> sections in the gpudb.conf file,
    /// with the key exception of the 'rankN.host' settings in the Network
    /// section that will be determined by <paramref name="hosts" /> instead.
    /// Though many of these configuration parameters typically are affixed
    /// with 'rankN' in the gpudb.conf file (where N is the rank number), the
    /// 'N' should be omitted in <paramref name="config_params" /> as the new
    /// rank number(s) are not allocated until the ranks have been added to the
    /// cluster. Each entry in this array corresponds to the entry at the same
    /// index in the <paramref name="hosts" />. This array must either be
    /// completely empty or have the same number of elements as the <paramref
    /// name="hosts" />.  An empty <paramref name="config_params" /> array will
    /// result in the new ranks being set with default parameters.</param>
...
public class AdminAddRanksResponse : KineticaData
{
    ...
    /// in the same order as the ranks in <see
