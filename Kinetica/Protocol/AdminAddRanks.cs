...
/// <para>* <see cref="hosts" /> would be an array including 172.123.45.67 in
...
/// <para>* <see cref="config_params" /> would be an array of maps, with each
/// map corresponding to the ranks being added in <see cref="hosts" />. The key
/// of each map would be the configuration parameter name and the value would
/// be the parameter's value, e.g. '{"rank.gpu":"1"}'</para>
...
public class AdminAddRanksRequest : KineticaData
{
    ...
    {
        /// <summary>If <see cref="Options.TRUE">TRUE</see>, only validation
    ...
    /// cref="hosts" /> could look like '["172.123.45.67", "172.123.45.67"]'.
    ...
    /// section that will be determined by <see cref="hosts" /> instead. Though
    /// many of these configuration parameters typically are affixed with
    /// 'rankN' in the gpudb.conf file (where N is the rank number), the 'N'
    /// should be omitted in <see cref="config_params" /> as the new rank
    ...
    /// index in the <see cref="hosts" />. This array must either be completely
    /// empty or have the same number of elements as the <see cref="hosts" />.
    /// An empty <see cref="config_params" /> array will result in the new
    /// ranks being set with default parameters.</para></remarks>
