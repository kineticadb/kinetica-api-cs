...
public class QueryGraphRequest : KineticaData
{
    ...
    public struct Options
    {
        ...
        /// as the WKT column of the adjacency table.</summary>
        /// <remarks><para>Supported values:</para>
        /// <list type="bullet">
        ///     <item>
        ///         <term><see cref="QueryGraphRequest.Options.TRUE">TRUE</see>
        ...
        /// bandwidth.</para></remarks>
        public const string SERVER_ID = "server_id";

        /// <summary>When specified (&gt;0 and &lt;=256), limits the number of
        /// char length on the output tables for string based nodes.</summary>
        /// <remarks><para>The default length is 64. The default value is '64'.
        /// </para></remarks>
    ...
    /// QUERY_NODE_ID', or expressions, e.g., 'ST_MAKEPOINT(table.x, table.y)
    /// AS QUERY_NODE_WKTPOINT'. Multiple values can be provided as long as the
    /// same identifier is used for all values. If using raw values in an
    ...
    /// must match across the combination. The default value is an empty List.
    /// </para></remarks>
    ...
    /// node.</summary>
    /// <remarks><para>Also known as number of hops. For example, if it is set
    /// to '2', the edge(s) directly attached to the queried node(s) will be
    ...
    /// <summary>Constructs a QueryGraphRequest object with default parameters.
    /// </summary>
    ...
    /// parameters.</summary>
    ///
    /// <param name="graph_name">Name of the graph resource to query.</param>
    /// <param name="queries">Nodes or edges to be queried specified using <a
    /// href="../../../graph_solver/network_graph_solver/#query-identifiers"
    ...
    /// values can be provided as long as the same identifier is used for all
    /// values. If using raw values in an identifier combination, the number of
    ...
    /// must match across the combination. The default value is an empty List.
    /// </param>
    /// <param name="adjacency_table">Name of the table to store the resulting
    /// adjacencies, in [schema_name.]table_name format, using standard <a
    /// href="../../../concepts/tables/#table-name-resolution"
    /// target="_top">name resolution rules</a> and meeting <a
    /// href="../../../concepts/tables/#table-naming-criteria"
    ...
    /// href="../../../graph_solver/network_graph_solver/#query-identifiers"
    /// target="_top">query identifier</a> is used in <paramref name="queries"
    /// />, then two additional columns will be available: 'PATH_ID' and
    /// 'RING_ID'. See <a
    /// href="../../../graph_solver/network_graph_solver/#using-labels"
    /// target="_top">Using Labels</a> for more information. The default value
    /// is ''.</param>
    /// <param name="rings">Sets the number of rings around the node to query
    /// for adjacency, with '1' being the edges directly attached to the
    /// queried node. Also known as number of hops. For example, if it is set
    /// to '2', the edge(s) directly attached to the queried node(s) will be
    /// returned; in addition, the edge(s) attached to the node(s) attached to
    /// the initial ring of edge(s) surrounding the queried node(s) will be
    /// returned. If the value is set to '0', any nodes that meet the criteria
    /// in <paramref name="queries" /> and <paramref name="restrictions" />
    /// will be returned. This parameter is only applicable when querying
    /// nodes. The default value is 1.</param>
...
public class QueryGraphResponse : KineticaData
{
    ...
    /// requested (if using QUERY_EDGE_ID or QUERY_NODE1_ID and QUERY_NODE2_ID
    /// in the input) or two node IDs per edge requested (if using
    /// QUERY_NODE_ID in the input).</summary>
    ...
    /// QUERY_NODE2_NAME in the input) or two node IDs per edge requested (if
    /// using QUERY_NODE_NAME in the input).</summary>
    public IList<string> adjacency_list_string_array { get; set; } = new List<string>();

    /// <summary>The adjacency entity WKTPOINT or WKTLINE ID: either edge IDs
    /// per node requested (if using QUERY_EDGE_WKTLINE or QUERY_NODE1_WKTPOINT
    /// and QUERY_NODE2_WKTPOINT in the input) or two node IDs per edge
    /// requested (if using QUERY_NODE_WKTPOINT in the input).</summary>
