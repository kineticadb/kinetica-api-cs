...
public class CreateProcRequest : KineticaData
{
    ...
    {
        /// <summary>Input table data will be divided into data segments that
        /// are distributed across all nodes in the cluster, and the proc
        /// command will be invoked once per data segment in parallel.
        /// </summary>
        /// <remarks><para>Output table data from each invocation will be saved
        /// to the same node as the corresponding input data.</para></remarks>
    ...
    public struct Options
    {
        ...
        /// <remarks><para>0 allows unlimited concurrency. The default value is
        /// '0'.</para></remarks>
        ...
        /// </summary>
        /// <remarks><para>Must be an existing environment, else an error will
    ...
    ///         to the same node as the corresponding input data.</description>
    ///     </item>
    ///     <item>
    ///         <term><see
    ...
    /// <remarks><para>The keys of the map are file names, and the values are
    /// the binary contents of the files. The file names may include
    /// subdirectory names (e.g. 'subdir/file') but must not resolve to a
    /// directory above the root for the proc.</para>
    /// <para>Files may be loaded from existing files in KiFS. Those file names
    /// should be prefixed with the uri kifs:// and the values in the map
    /// should be empty. The default value is an empty Dictionary.</para>
    /// </remarks>
    ...
    /// <summary>Constructs a CreateProcRequest object with default parameters.
    /// </summary>
    ...
    /// parameters.</summary>
    ///
    /// <param name="proc_name">Name of the proc to be created. Must not be the
    ...
    /// 'subdir/file') but must not resolve to a directory above the root for
    /// the proc.
    ...
    /// <param name="command">The command (excluding arguments) that will be
    /// invoked when the proc is executed. It will be invoked from the
    /// directory containing the proc <paramref name="files" /> and may be any
    /// command that can be resolved from that directory. It need not refer to
    /// a file actually in that directory; for example, it could be 'java' if
    /// the proc is a Java application; however, any necessary external
    /// programs must be preinstalled on every database node. If the command
    /// refers to a file in that directory, it must be preceded with './' as
    /// per Linux convention. If not specified, and exactly one file is
    /// provided in <paramref name="files" />, that file will be invoked. The
    /// default value is ''.</param>
    /// <param name="args">An array of command-line arguments that will be
    /// passed to <paramref name="command" /> when the proc is executed. The
