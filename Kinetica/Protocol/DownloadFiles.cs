...
public class DownloadFilesRequest : KineticaData
{
    ...
    /// <summary>An array of the file names to download from KiFS.</summary>
    /// <remarks><para>File paths may contain wildcard characters after the
    /// KiFS directory delimiter.</para>
    /// <para>Accepted wildcard characters are asterisk (*) to represent any
    /// string of zero or more characters, and question mark (?) to indicate a
    /// single character.</para></remarks>
    ...
    /// <summary>Constructs a DownloadFilesRequest object with the specified
    /// parameters.</summary>
    ///
    /// <param name="file_names">An array of the file names to download from
    /// KiFS. File paths may contain wildcard characters after the KiFS
    /// directory delimiter.
    /// Accepted wildcard characters are asterisk (*) to represent any string
    /// of zero or more characters, and question mark (?) to indicate a single
    /// character.</param>
    /// <param name="read_offsets">An array of starting byte offsets from which
    /// to read each respective file in <paramref name="file_names" />. Must
    /// either be empty or the same length as <paramref name="file_names" />.
    /// If empty, files are downloaded in their entirety. If not empty,
    /// <paramref name="read_lengths" /> must also not be empty.</param>
    /// <param name="read_lengths">Array of number of bytes to read from each
    /// respective file in <paramref name="file_names" />. Must either be empty
    /// or the same length as <paramref name="file_names" />. If empty, files
    /// are downloaded in their entirety. If not empty, <paramref
    /// name="read_offsets" /> must also not be empty.</param>
